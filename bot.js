...
app.get('/login', (req, res) => {
  ...
  app.get('/dashboard', ensureAuthenticated, (req, res) => {
    // Assuming you have some way of retrieving or defining botSettings
    const botSettings = {
      apiKey: 'your-api-key',
      // Add other settings as needed
    };
  
    // Replace this with your logic to retrieve or define the 'users' variable
    const users = [];// Replace with your actual user data
  ...
  app.put('/admin/botkey', async (req, res) => {
    try {
      const botkey = req.body.botkey;
      // Find the first document in the BotKey collection and update the 'key' field
      const updatedBotKey = await BotKey.findOneAndUpdate({}, { $set: { key: botkey } }, { upsert: true, new: true });
      res.json({ message: 'Bot key updated successfully', data: updatedBotKey });
    } catch (error) {
      res.status(500).json({ message: 'Error updating bot key', error: error.message });
    }
  });
  app.get('/user-management', (req, res) => {
    // Assuming you have some way of retrieving or defining userData
    const userData = [
    ...
    res.render(path.join(__dirname, 'views', 'admin', 'userManagement.ejs'), {
      user: req.user,
      users: userData, // Pass the 'users' variable to the template
    });
  });
...
bot.start((ctx) => {
    ...
    ctx.reply('Welcome to the WEATHER UPDATE bot!\nThis is a Weather Update bot made by Aayush to get the weather information of a particular city of your choice... \n\n\nTap /help - To Get help with commands\n\nhope ');
});
